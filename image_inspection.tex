...
immense, in particular for identifying patterns easily spotted by eye yet tend
to evade most modern automated image quality assessment protocols.
...
detectors, electronice, the night sky, etc.  This makes eyeball inspection
particulalry valuable in the early days of comissioning.  Fortuitously, the
amount of energy and enthusiasm from internal project members are at extrememly
high levels at this stage, so there is no shortage of voluntary effort for
human visial inpsection for the commissioning phase of the LSSTComCam.  This
effort has largely proceded by an informal see-something-say-something scheme,
with many users posting their latest findings on the internal staff Slack
channels (namely the \#sciunit-image=inspection channel, but also prominently
in other channels, \#sciunit-lsb, \#validation-team, \#embargo-beautiful-images,
\#ops-satellites, to name a few).
...
\item \textbf{ghosts}: largely seen around bright stars (including those falling
  ouside of the FOV) and attributed to multi-bounce reflections off of the focal
...
\item \textbf{stray/scattered light}: prominent ring-shaped waffle/courderoy-like
  features seen early on were identified as originating from a bliking light on a
...
\item \textbf{kettlebell \& trefoil-shaped PSFs}: see Section~\ref{sec:aos_commissioning}.
  ...
  irregularly shaped PSFs.  It was noted that the preferred ``piff'' algorightm (which
  we use as the final characterizing model) was performing worse that the ``psfex''
  ...
  improved.  Given the incredible work of the AOS team, we are already confident that
  we can switch back to our preferred configurarion.  (This has also spurred a study
  into the reason why ``piff'' struggled with these shapes and will hopefully lead to
  improvements there, see Section~\ref{sec:delivered_image_quality_and_psf}).

\item \textbf{background subtraction issues}: see Section~\ref{sec:low_surface_brightness}

\item \textbf{repeated patchwork gradients}: I have admittedly have yet to read the 114+
  reply thread for this one, but see issues like crosstalk and electronics readouts, etc.,
  so most likely to be further assessed in Section~\ref{sec:isr}-related work.

\item \textbf{trailed bright sources}: due to tracking errors.  Difficult to idendify
  with automated image quality metrics (some ideas about using AI to identify tracking
  error-based image degradation, but we also hope to be able to confidently rely on
...
HIPS maps produced regularly during the nightly validation and DRP runs.  Such images
are invaluable for highlighting myriad issues at the coadd level (often indicating a
need to drill-down to the visit-level for a full diagnosis, but providing significant
clues on where to look first).
